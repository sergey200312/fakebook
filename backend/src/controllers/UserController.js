...
const getProfileDetails = asyncHandler(async (req, res, next) => {
    ...
    const subscribersCount = user.subscriptions.length;

    return res.status(200).json({ message: 'Профиль успешно найден', user, friendsCount, subscriptionsCount, sentRequestStatus, friendStatus, receivedStatus, subscribersCount })
...
const getReceivedFriendRequests = asyncHandler(async (req, res, next) => {
    ...
    const receivedFriendReq = await User.findById(userId).select('friendRequests.received').populate('friendRequests.received').exec();
